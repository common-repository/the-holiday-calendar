...
class thc_calendar {
	...
	static function draw_calendar($month,$year,$sundayFirst, $events, $countryIso){
		...
		/* table headings */
		$headings = thc_translation_helper::get_week_names();	
		
		for($x = 0; $x < count($headings); $x++):
			$headings[$x] = mb_substr($headings[$x], 0, 1);
		endfor;
		...
		for($list_day = 1; $list_day <= $days_in_month; $list_day++):
			...
			$calendar .= '<td class="thc-calendar-day' . $highlightCode . '">';
				...
				if($numberOfEvents > 0)
				{
					...
					$url = add_query_arg(array('thc-date' => $currentDate), $url);
					
					if($countryIso != null)
					{
						$url = add_query_arg(array('thc-country' => $countryIso), $url);
					}
					
					if(null !== http_get_helper::get_month())
					{
						$url = add_query_arg(array('thc-month' => htmlspecialchars(http_get_helper::get_month())), $url);
					}
